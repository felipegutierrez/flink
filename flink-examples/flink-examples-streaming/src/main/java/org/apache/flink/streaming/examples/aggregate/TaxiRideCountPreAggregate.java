...
public class TaxiRideCountPreAggregate {
	public static void main(String[] args) throws Exception {
		...
		DataStream<Tuple2<Long, Long>> tuples = rides01.union(rides02).map(new TokenizerMap()).name(OPERATOR_TOKENIZER).uid(OPERATOR_TOKENIZER);
		...
		if (preAggregationWindowCount == 0) {
			...
			preAggregatedStream = tuples
				...
				.name(OPERATOR_PRE_AGGREGATE).uid(OPERATOR_PRE_AGGREGATE).disableChaining();
		}
