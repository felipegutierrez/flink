...
public class TaxiRideCountPreAggregate {
	...
	private static class TaxiRideTimeCountPreAggregateFunction
		...
		@Override
		public Tuple2<Long, Long> addInput(@Nullable Tuple2<Long, Long> value, Tuple3<Long, Long, Long> input) throws Exception {
			if (value == null) {
		...
		@Override
		public void collect(Map<Long, Tuple2<Long, Long>> buffer, Collector<Tuple3<Long, Long, Long>> out) {
	...
	private static class TaxiRideTimeCountPreAggregateConcurrentFunction
		...
		@Override
		public void collect(ConcurrentMap<Long, Tuple2<Long, Long>> buffer, Collector<Tuple3<Long, Long, Long>> out) {
			for (Map.Entry<Long, Tuple2<Long, Long>> entry : buffer.entrySet()) {
				out.collect(Tuple3.of(entry.getKey(), entry.getValue().f0, entry.getValue().f1));
			}
		}
	...
	private static class SumTimeReduceFunction implements ReduceFunction<Tuple3<Long, Long, Long>> {
		@Override
		public Tuple3<Long, Long, Long> reduce(Tuple3<Long, Long, Long> value1, Tuple3<Long, Long, Long> value2) {
			// return only the largest timestamp
